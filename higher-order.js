...
// This code Checker for this file is broken !!!!!!!!!!!!!!!!!!!

function copyEvens(element) {
  if(element % 2 === 0) {
    return element;
  }
}
...
//Code Here
let evenNumbers = mixedNumbers.filter(copyEvens);
...
function addPopulation(total, element) {
  total = total + element;
  return total;
}

//Code Here
let totalPopulation = populations.reduce(addPopulation);
...
function monsterIq(element) {
  return element.CP > 200;
}

//Code Here
let myStrongest= monstersInYourPocket.filter(monsterIq)
...
*/
function calcOrderTotals(element) {
  var afterTaxTotal = element.price * element.tax;
  return afterTaxTotal;
}


let orderTotals = orders.map(calcOrderTotals);// Code here
